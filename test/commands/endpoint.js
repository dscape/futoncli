...
specify('futon endpoint add test http://localhost:5984/test', function (assert) {
  assert.expect(6);

  run('futon endpoint add test http://localhost:5984/test')
  .expect(function (response) {
    futon_ok(assert, response);

    var conf = get_config();
    assert.ok(conf['endpoints']);
    assert.ok(conf['endpoints']['test']);
    assert.equal(conf['endpoints']['test'], 'http://localhost:5984/test');
  })
  .end(function () {
    assert.ok(true);
  });
...
specify('futon endpoint set test', function (assert) {
  assert.expect(5);

  run('futon endpoint set test')
  .expect(function (response) {
    futon_ok(assert, response);
    ...
    assert.ok(conf['endpoint']);
    assert.equal(conf['endpoint'], 'http://localhost:5984/test');
  })
  .end(function () {
    assert.ok(true);
  });
...
specify('futon endpoint delete test', function (assert) {
  assert.expect(4);

  run('futon endpoint delete test')
  .expect(function (response) {
    futon_ok(assert, response);

    var conf = get_config();
    assert.equal(typeof conf['endpoints']['test'], 'undefined');
  })
  .end(function () {
    assert.ok(true);
  });
